...
async function yt(url, quality, type, bitrate, server = 'en68') {
  ...
  let res = await post(`https://www.y2mate.com/${server}/download-youtube`, {
    url,
    q_auto: 0,
    ajax: 1
  })
  ...
  let res2 = await post(`https://www.y2mate.com/${server}/convert-youtube`, {
    type: 'youtube',
    _id: id[1],
    v_id: ytId[1],
    ajax: '1',
...
module.exports = {
   ...
   * @param {String} server (avaiable: `id4`, `en60`, `en61`, `en68`)
   */
  yta(url, resol = '128kbps', server = 'en358') { return yt(url, resol, 'mp3', resol.endsWith('kbps') ? resol.replace(/kbps/g, '') : resol, server) },
  /**
   * Download YouTube Video as Video via y2mate
   * @param {String} url YouTube Video URL
   * @param {String} server (avaiable: `id4`, `en60`, `en61`, `en68`)
   */
  ytv(url, resol = '360p', server = 'en358') { return yt(url, resol, 'mp4', resol.endsWith('p') ? resol.replace(/p/g, '') : resol, server) },
